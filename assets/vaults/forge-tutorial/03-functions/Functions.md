...
There are two snippets here. The reusable one is `excited`:
...
- `[[excited]](word="hooray")` **calls your `excited` snippet**, handing it the
  input `word` set to `"hooray"`.
...
snippet — nothing new to learn beyond what a snippet already is.
...
1. In the file list on the left, right-click `excited.md` and choose **Make a
   copy**. Rename the copy to `question.md`.
2. Open `question.md` and change its English to: `Give back word plus "?".`
3. Open `cheer`, change `[[excited]]` to `[[question]]`, and **Forge** 🔥 it.
   You'll see `hooray?`.
