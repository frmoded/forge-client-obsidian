...
```python
def compute(context):
    harm = context.compute("form")
    phrase_a1 = context.compute("vocal_phrase_a")
    phrase_a2 = context.compute("vocal_phrase_a")
    phrase_b = context.compute("vocal_phrase_b")
    vocal_line = sequence(phrase_a1, phrase_a2, phrase_b)
    return voices(harm, vocal_line)
```
