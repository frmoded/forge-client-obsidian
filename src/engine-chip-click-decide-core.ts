...
// them — cohort never discovered EngineChipView. New 4th param
// `vaultNoteRawContent` lets the caller hand in the resolved file's
// markdown; if the classifier says 'forensic', we open EngineChipView
...
export type EngineChipClickDecision =
  | { action: 'open-engine-chip'; chipName: string; shadowToCleanup: boolean }
 ...
 *   default and routes to EngineChipView with shadowToCleanup=true.
 */
export function decideEngineChipClick(
  ...
  if (vaultHasNoteFor) {
    ...
    // garbage from a pre-v0.2.206 Cmd-click), prefer EngineChipView
  ...
  return { action: 'open-engine-chip', chipName: bare, shadowToCleanup: false };
}
