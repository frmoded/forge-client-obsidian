...
// v0.2.272 drain 1320 — chipSourcesFor retired with the `_chips.md`
// schema; tests deleted (5 dead tests).
...
// v0.2.272 drain 1320 — autoDeriveChips, parseChipsV2Config,
// mergeChipsWithOverrides + ChipsV2Config type retired with the
// `_chips.md` schema (`_chips.md` files themselves were retired in
// drain 1300 v0.2.259; the reader stack in drain 1310 v0.2.263; the
// schema surface here). ~17 dead tests deleted.
...
// v0.2.272 drain 1320 — end-to-end pipeline tests (autoDeriveChips →
// mergeChipsWithOverrides) deleted. Live palette pipeline coverage
// lives in palette-discovery-core.test.ts.
//
// v0.2.272 drain 1320 — mergeChipsConfigsWalkUp walk-up tests deleted
// with the `_chips.md` schema retirement (7 dead tests).
